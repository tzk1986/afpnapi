...
本版新增重点（v1.37.0 ~ v1.40.2）：
- **项目脚手架（v1.39.0 引入，v1.39.1 起默认开启）**：模板化创建接口测试项目（模板 → 项目 → 集合 → 执行 → 追溯 → 导出全链路），3 页面（`/projects` 列表 / 创建 / 详情）+ 16 API（v1.40.0 起 A16 支持删除用户模板，创建页卡片带删除按钮；创建页另有「上传新模板 / 下载示例模板」入口，小白可下载示例改完即传，见操作手册 §0.9）；内置 3 套模板（api_basic / api_esp / api_payment），支持用户上传模板（同名覆盖内置）；A9 一键执行与手工上传执行**同队列**复用既有执行链，执行历史懒对账（重启后仍能收敛 done/failed）；用例追溯表（CSV 导出 + zip 整项导出经 `/exports/` 下载）；编辑采用 CAS 乐观锁（冲突返回最新对象供前端合并）。开关 `ENABLE_PROJECT_SCAFFOLD` 默认 `true`（v1.39.1 起）；显式设 `false` 时页面与 API 均 403、零文件系统痕迹（方案：`方案讨论/2026-09-02-项目脚手架功能实施方案-v5.md`）
