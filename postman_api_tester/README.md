...
本版新增重点（v1.37.0 ~ v1.40.4）：
- **项目脚手架（v1.39.0 引入，v1.39.1 起默认开启）**：模板化创建接口测试项目（模板 → 项目 → 集合 → 执行 → 追溯 → 导出全链路），3 页面（`/projects` 列表 / 创建 / 详情）+ 16 API（v1.40.0 起 A16 支持删除用户模板，创建页卡片带删除按钮；创建页另有「上传新模板 / 下载示例模板」入口，小白可下载示例改完即传，见操作手册 §0.9；v1.40.3 起上传派生 id 撞内置时自动避让随机 id，保证上传模板必可删）；内置 3 套模板（api_basic / api_esp / api_payment），支持用户上传模板（同名覆盖内置）；A9 一键执行与手工上传执行**同队列**复用既有执行链，执行历史懒对账（重启后仍能收敛 done/failed）；用例追溯表（CSV 导出 + zip 整项导出经 `/exports/` 下载）；编辑采用 CAS 乐观锁（冲突返回最新对象供前端合并）。开关 `ENABLE_PROJECT_SCAFFOLD` 默认 `true`（v1.39.1 起）；显式设 `false` 时页面与 API 均 403、零文件系统痕迹（方案：`方案讨论/2026-09-02-项目脚手架功能实施方案-v5.md`）
...
- **任务级自定义判定规则入口（v1.37.23）**：首页「判定条件配置」区新增自定义判定规则行编辑（path/op/expected，13 操作符），对本次执行的每个接口生效，与接口级 `x_judgment_rules` 合并评估（任务级在前）；勾选「关闭一级判定」时任务级规则一并不执行
- **自定义一级判定规则（v1.37.22）**：接口级 `request.x_judgment_rules`（与断言同三元组结构），内置判定通过后评估，任一失败判 FAILED 且短路断言；关闭一级判定开关与自定义规则联动失效（状态码兜底保留）；编辑器断言页签「一级判定」小节新增自定义规则列表区
- **执行详情断言表布局修复（v1.37.21）**：弹窗断言结果表改为 fixed 布局 + colgroup 定宽 + 单元格内换行，长 expected/actual/说明不再撑破详情弹窗排版（v1.37.20 落盘修复后可见但排版被长文本破坏）
- **报告断言明细落盘修复（v1.37.20）**：meta 写入白名单补 `assertion_results`/`assertion_engine_error`（v1.37.16 仅接通读取端与前端弹窗，写侧漏盘导致在线「执行详情」断言表永远为空、analytics 断言覆盖率恒 0），修复后新生成报告的详情弹窗正常显示断言结果表；存量旧报告无法补显
- **夜间走查修复（v1.37.19）**：编辑器「关闭一级判定」确认弹窗同步识别集合共享断言（`x_shared_assertions`），修复仅配置共享断言的集合被误提示「未配置任何断言」
- **接口级一级判定开关 + 集合级共享断言（v1.37.18）**：编辑器断言页签新增「一级判定」小节（errCode/message 三态：继承/强制开启/强制关闭 + 共享断言豁免勾选），单接口可持久关闭一级判定让断言接管成败——混合正/异常用例集合统一回归管理；新增 Collection 根级 `x_shared_assertions` 共享断言（作用于全部接口、与自有 AND 合并）；修复编辑器导入→保存往返静默丢弃 `x_enable_*`/`x_success_*` 判定字段的阻塞性 bug
...
本版新增重点（v1.34.0）：
- **UI 执行登录状态持久化**：认证档案 CRUD + 无头执行预加载 Cookie + 编辑器认证档案选择 + 录制 Cookie 导出

本版新增重点（v1.33.27）：
- **语义化选择器断言 `assert_element_exists`**：支持 `button:查看订单` 格式，按角色+文字精确断言元素存在

本版新增重点（v1.33.26）：
- **截图策略统一**：失败步骤自动截图（无论是否勾选），两种执行模式行为一致

本版新增重点（v1.33.22）：
- **代码质量优化**：消除硬编码端口号（使用 `REPORT_SERVER_PORT` 配置常量）、登录按钮检测支持中英文国际化

本版新增重点（v1.33.21）：
- **纯文本断言 `assert_text_exists`**：新增无需选择器的纯文本断言动作，用户只需输入要查找的文字即可验证页面是否包含该文本，大幅简化断言配置

本版新增重点（v1.33.20）：
- **PNG 截图格式**：后端截图保存接口改用 Playwright 将 HTML 快照转换为 PNG 静态图片，避免 iframe 中 JavaScript 重新执行导致的"动态图"问题

本版新增重点（v1.33.19）：
- **截图字段同步修复**：回放引擎在截图保存后设置 `result.screenshot = 'saved'`，确保 `result.json` 中的 `screenshot` 字段正确标记，报告页面能显示成功步骤的截图链接

本版新增重点（v1.33.18）：
- **修复重复代码 bug**：修复 v1.33.16 中引入的 `ui_recorder_inject.py` 重复代码问题，清理元素未找到时的冗余回调逻辑

本版新增重点（v1.33.17）：
- **截图接口测试覆盖**：新增 10 个单元测试，覆盖截图保存接口的成功/失败步骤、向后兼容性、边界条件等场景

本版新增重点（v1.33.16）：
- **截图状态区分**：修复成功步骤和失败步骤截图文件名冲突问题，现在根据步骤状态分别保存为 `step_N.html`（成功）或 `step_N_fail.html`（失败），确保成功截图能正确显示

本版新增重点（v1.33.15）：
- **浏览器回放截图修复**：修复浏览器回放模式下步骤勾选"截图"后未执行截图的问题，现在回放引擎会正确捕获步骤截图
...
本版新增重点（v1.33.8）：
- **测试覆盖增强**：新增 ui_execution_routes 单元测试（7 个），覆盖设置获取/更新/重置、执行状态查询、执行列表等 API
...
本版新增重点（v1.33.4）：
- **截图显示修复**：失败步骤截图支持 HTML 格式，前端自动选择 `<img>` 或 `<iframe>` 渲染

本版新增重点（v1.33.3）：
- **回放状态残留修复**：修复多次执行回放时只运行最后几步的问题（清除 `savedReplayState` 和 `sessionStorage` 残留状态）；用例列表页增加"查看录制"入口

本版新增重点（v1.33.0）：
- **代码走查优化**：8 项安全/并发/架构整改 — `_active_jobs` 加锁、Cookie 日志脱敏、变量存储 I/O 提取、handlers 文件 I/O 下沉、录制会话管理类合并、report_server SPA 兜底迁移等

本版新增重点（v1.31.14）：
- **每晚自动升级**：ruff 检查通过，无代码变更
...
本版新增重点（v1.31.10）：
- **步骤详情翻页移至右下角**：筛选按钮保留在表格上方，翻页控件移至表格右下角，布局更清晰

本版新增重点（v1.31.9）：
- **执行报告 UI 优化**：步骤表格全列固定宽度 + table-layout:fixed 防溢出；选择器/值/错误信息列点击弹窗显示完整内容；状态列宽适配中文；时间格式 JS 轮询一致性修复；进度条文字精简；报告列表页状态标签、筛选按钮、步骤统计格式、耗时显示等多项修复
...
- **store.update_step 幂等修复**：post-check 回写步骤状态时不再被幂等检查丢弃，确保 new_tab 失败状态正确写入 store

本版新增重点（v1.31.5）：
- **headless 执行进度条实时更新**：修复执行期间进度条不更新问题，worker 子进程改为每步实时输出 JSON 行，父进程逐行读取并即时更新 store，前端轮询可见中间进度

本版新增重点（v1.31.4）：
- **ruff 全量修复**：修复 26 个预存 ruff 错误（E722 裸 except ×5、F841 未使用变量 ×15、F401 未使用导入 ×1、E402 导入顺序 ×2、E731 lambda 赋值 ×1），项目零警告通过

本版新增重点（v1.31.3）：
- **每晚自动升级**：ruff 格式化

本版新增重点（v1.31.2）：
- **new_tab 导航验证**：跳转后验证 URL 匹配 + 等待 networkidle 确保 SPA 渲染完成，导航失败则终止测试
- **启动 loading 可见性修复**：running 状态且无步骤完成时也显示"浏览器启动中……"

本版新增重点（v1.31.1）：
- **浏览器启动 loading 界面**：headless 执行时显示"浏览器启动中……"提示，启动时间从总耗时中剔除
- **去除 networkidle 等待**：启动阶段不再等待网络空闲，节省约 5 秒

本版新增重点（v1.31.0）：
- **步骤截图功能**：用例编辑器每个步骤增加"截图"勾选项，headless 执行时自动截图并在报告中显示
- **greenlet 线程错误修复**：headless 执行改为独立子进程运行 Playwright，彻底解决 Waitress 线程池与 greenlet 的线程冲突
- **Windows 中文编码修复**：worker 子进程输出强制 UTF-8 编码，修复中文选择器在报告中的乱码问题
- **headless 执行性能优化**：步骤间延迟 500→200ms；同页面跳步延迟；`is_visible()` 快速路径；执行时间 ~26s → ~16s

本版新增重点（v1.30.80）：
- **Vite 副作用导入改写**：`_rewrite_js_imports` 新增 `import"./chunk.js"` 副作用导入（无 `from` 关键字）改写，修复 chunk 文件中 vendor 依赖 404 导致页面灰屏和 new_tab 加载失败
...
- 操作手册.md（完整手册；项目脚手架零基础从开头 §0.0 保姆级跟做读起）
- 快速命令参考.md（常用命令速查）
- 最终交付清单.md（交付说明）
...
- `ENABLE_ASSERTIONS`：是否显示 ad-hoc 页面的 JSONPath 断言配置入口（默认 `True`；仅控制 UI 显隐，不影响执行——断言仅在用例配置 `x_assertions` 且安装 jsonpath-ng 时才执行）
