...
本版新增重点（v1.37.0 ~ v1.37.10）：
- **API 响应格式统一（L-1 系列）**：ui_recorder / retry / collection / job 入队 / server / collection_editor / report_meta / test_proxy 等 handler 的成功响应统一为 `{code, message, data, timestamp}` 包装格式，前端统一入口解包；`/health`、`/api/log-metrics`、`/api/run-postman-status`、`/api/reports`、`/api/report-meta` 按决策保留原始格式（监控/稳定接口语义）
