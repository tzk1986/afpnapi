...
本版新增重点（v1.37.0 ~ v1.37.15）：
- **JSONPath 断言全链路生效（v1.37.14~15）**：修复 parser/ad-hoc 断链（此前断言输入被静默丢弃），可视化编辑器新增「断言」页签（JSONPath + 13 操作符，随 `ENABLE_ASSERTIONS` 显隐）；⚠️ 存量带 `x_assertions` 集合的断言自 v1.37.14 起真正执行
