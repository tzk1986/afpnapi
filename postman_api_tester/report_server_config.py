...
REPORT_EXPORT_DEFAULT_SCOPE = (
    _cfg_str("REPORT_EXPORT_DEFAULT_SCOPE", "full").lower() or "full"
)
...
REPORT_EXPORT_INCLUDE_AUTH_DEFAULT = _cfg_bool(
    "REPORT_EXPORT_INCLUDE_AUTH_DEFAULT", False
)
REPORT_EXPORT_CHANNEL_MODE = (
    _cfg_str("REPORT_EXPORT_CHANNEL_MODE", "auto").lower() or "auto"
)
...
ADHOC_DEFAULT_COLLECTION_NAME = (
    _cfg_str("ADHOC_DEFAULT_COLLECTION_NAME", "报告中心临时测试") or "报告中心临时测试"
)
...
ENABLE_ASSERTIONS = _cfg_bool("ENABLE_ASSERTIONS", True)
ENABLE_DATA_DRIVEN = _cfg_bool("ENABLE_DATA_DRIVEN", True)
ENABLE_VARIABLE_EXTRACTION = _cfg_bool("ENABLE_VARIABLE_EXTRACTION", True)
...
REPORT_ANALYTICS_TREND_LIMIT_DEFAULT = _cfg_int(
    "REPORT_ANALYTICS_TREND_LIMIT_DEFAULT", 20
)
...
REPORT_ANALYTICS_HISTOGRAM_BUCKETS = _cfg_str(
...
QUALITY_SCORE_ASSERTION_MISSING_PENALTY = max(
    0, _cfg_int("QUALITY_SCORE_ASSERTION_MISSING_PENALTY", 2)
)

# 报告扫描排除目录（逗号分隔，默认排除 old）
REPORT_SCAN_EXCLUDE_DIRS: List[str] = [
    d.strip()
    for d in _cfg_str("REPORT_SCAN_EXCLUDE_DIRS", "old").split(",")
    if d.strip()
]

ENVIRONMENTS: Dict[str, EnvironmentConfig] = _normalize_environments(
    _cfg_dict("ENVIRONMENTS", {})
)
...
SUCCESS_ERR_CODES_SET: frozenset = _parse_success_list(
    _cfg_str("SUCCESS_ERR_CODES", "0")
)
SUCCESS_MESSAGES_SET: frozenset = _parse_success_list(
    _cfg_str("SUCCESS_MESSAGES", "success")
)
...
ENABLE_VARIABLE_FUNCTIONS: bool = _cfg_bool("ENABLE_VARIABLE_FUNCTIONS", True)
GLOBAL_VARIABLES_FILE: str = _cfg_str("GLOBAL_VARIABLES_FILE", "variables.json")
GLOBAL_VARIABLES_MAX_COUNT: int = _cfg_int("GLOBAL_VARIABLES_MAX_COUNT", 1000)
...
LOG_ALERT_ERROR_WINDOW_SECONDS = max(
    60, _cfg_int("LOG_ALERT_ERROR_WINDOW_SECONDS", 300)
)
LOG_ALERT_ERROR_RATE_THRESHOLD_PER_MIN = max(
    0.0, _cfg_float("LOG_ALERT_ERROR_RATE_THRESHOLD_PER_MIN", 10.0)
)
LOG_FILE = _cfg_str("LOG_FILE", "logs/report_server.log")
