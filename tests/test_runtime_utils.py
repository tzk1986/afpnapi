...
class TestCheckpointFilePath:
    ...
    def test_custom_checkpoint_dir(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = checkpoint_file_path(
                tmpdir, "test.json", "abcdef1234567890", checkpoint_dir=custom
            )
    ...
    def test_special_chars_in_filename(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            result = checkpoint_file_path(
                tmpdir, "my file (1).json", "abcdef1234567890"
            )
            basename = Path(result).name
    ...
    def test_with_data_file(self) -> None:
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as pf:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                h2 = compute_collection_fingerprint(
                    pf.name, "", None, data_file=df.name
                )
            Path(df.name).unlink()
        Path(pf.name).unlink()
        assert h1 != h2
