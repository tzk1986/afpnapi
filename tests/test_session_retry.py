...
class TestRequestRetry:
    ...
    def test_retry_enabled_configures_adapter(self):
        ...
        with patch('postman_api_tester.config.ENABLE_REQUEST_RETRY', True), \
             patch('postman_api_tester.config.REQUEST_RETRY_TOTAL', 3), \
             patch('postman_api_tester.config.REQUEST_RETRY_BACKOFF_FACTOR', 1.0), \
             patch('postman_api_tester.config.REQUEST_RETRY_STATUS_FORCELIST', (429, 500, 502, 503, 504)):
            session = create_shared_session()
            adapter = session.get_adapter("http://example.com")
            # 应该配置了重试
    ...
    def test_retry_with_custom_status_codes(self):
        ...
        with patch('postman_api_tester.config.ENABLE_REQUEST_RETRY', True), \
             patch('postman_api_tester.config.REQUEST_RETRY_TOTAL', 2), \
             patch('postman_api_tester.config.REQUEST_RETRY_BACKOFF_FACTOR', 0.5), \
             patch('postman_api_tester.config.REQUEST_RETRY_STATUS_FORCELIST', custom_status):
            session = create_shared_session()
            adapter = session.get_adapter("http://example.com")
    ...
    def test_retry_backoff_factor_validation(self):
        ...
        with patch('postman_api_tester.config.ENABLE_REQUEST_RETRY', True), \
             patch('postman_api_tester.config.REQUEST_RETRY_TOTAL', 3), \
             patch('postman_api_tester.config.REQUEST_RETRY_BACKOFF_FACTOR', 0.0), \
             patch('postman_api_tester.config.REQUEST_RETRY_STATUS_FORCELIST', (500,)):
            # 0.0 是合法值，应该能正常创建
            session = create_shared_session()
            adapter = session.get_adapter("http://example.com")
    ...
    def test_retry_allowed_methods(self):
        """重试应覆盖常用 HTTP 方法。"""
        with patch('postman_api_tester.config.ENABLE_REQUEST_RETRY', True), \
             patch('postman_api_tester.config.REQUEST_RETRY_TOTAL', 1), \
             patch('postman_api_tester.config.REQUEST_RETRY_BACKOFF_FACTOR', 1.0), \
             patch('postman_api_tester.config.REQUEST_RETRY_STATUS_FORCELIST', (500,)):
            session = create_shared_session()
            adapter = session.get_adapter("http://example.com")
