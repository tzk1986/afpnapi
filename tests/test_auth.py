...
class TestExtractTokenFromPayload:
    ...
    def test_top_level_authorization(self) -> None:
        assert (
            _extract_token_from_payload({"authorization": "Bearer xxx"}) == "Bearer xxx"
        )
    ...
    def test_nested_access_token(self) -> None:
        assert (
            _extract_token_from_payload({"data": {"access_token": "nested_xyz"}})
            == "nested_xyz"
        )
    ...
    def test_no_token_fields_returns_none(self) -> None:
        assert (
            _extract_token_from_payload({"name": "test", "data": {"value": 1}}) is None
        )
...
class TestGetAuthToken:
    ...
    def _make_api(
        self, name: str = "Login", url: str = "/login", method: str = "POST"
    ) -> Dict[str, Any]:
    ...
    def test_uses_full_url_when_present(self) -> None:
        api = {
            "name": "Login",
            "full_url": "https://auth.example.com/login",
            "url": "/login",
            "method": "POST",
        }
        mock_response = MagicMock()
