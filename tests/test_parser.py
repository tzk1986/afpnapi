...
class TestParseRequest:
    ...
    def _make_parser(
        self, tmpdir: str, base_url: str = "https://api.example.com"
    ) -> PostmanApiParser:
...
class TestNormalizeApiName:
    ...
    def test_normal_name(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            parser = PostmanApiParser(path)
            assert (
                parser._normalize_api_name("Get Users", "GET", "/users") == "Get Users"
            )
    ...
    def test_question_mark_name_from_url(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            parser = PostmanApiParser(path)
            assert (
                parser._normalize_api_name("?", "POST", "/api/data") == "POST /api/data"
            )
...
class TestBuildUrlFromDict:
    ...
    def test_raw_url_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                parser._build_url_from_dict(url_dict) == "https://api.example.com/users"
            )
