...
def app() -> Flask:
    ...
    app.testing = True
    app.add_url_rule(
        "/api/global-variables", "gv_get", api_global_variables_get, methods=["GET"]
    )
    app.add_url_rule(
        "/api/global-variables", "gv_set", api_global_variables_set, methods=["POST"]
    )
    app.add_url_rule(
...
class TestApiGlobalVariablesGet:
    def test_disabled_returns_403(self, app: Flask) -> None:
        with patch(
            "postman_api_tester.handlers.global_variables_routes._get_file_path",
            return_value="",
        ):
...
class TestApiGlobalVariablesSet:
    ...
    def test_set_success(self, app: Flask, vars_file: str) -> None:
        with patch(
            ...
            client = app.test_client()
            resp = client.post(
                "/api/global-variables", json={"key": "mykey", "value": "myval"}
            )
            assert resp.status_code == 200
            data = resp.get_json()
