...
def _register_routes(app: Flask) -> None:
    ...
    app.add_url_rule(
        "/api/report-analytics/<report_name>", "analytics", api_report_analytics
    )
    app.add_url_rule(
    ...
    app.add_url_rule(
        "/api/report-results/<report_name>/<int:result_index>",
        "detail",
        api_report_result_detail,
    )
...
class TestApiReportResults:
    ...
    @patch(
        "postman_api_tester.report_repository.find_report",
        return_value={"report_name": "test"},
    )
    def test_success(
        self, mock_find: MagicMock, mock_payload: MagicMock, app: Flask
    ) -> None:
        """成功返回结果列表。"""
        _register_routes(app)
    ...
    def test_with_pagination(
        ...
        resp = app.test_client().get(
            "/api/report-results/test_report?page=2&page_size=10"
        )
        assert resp.status_code == 200
...
class TestApiReportAnalytics:
    ...
    @patch(
        "postman_api_tester.handlers.report_result_routes.ENABLE_REPORT_ANALYTICS",
        False,
    )
    ...
    @patch(
        "postman_api_tester.handlers.report_result_routes.ENABLE_REPORT_ANALYTICS", True
    )
    @patch(
        "postman_api_tester.report_repository.find_report",
        side_effect=FileNotFoundError(),
    ...
    @patch(
        "postman_api_tester.handlers.report_result_routes._repo_list_reports",
        return_value=[],
    )
    @patch(
...
class TestApiReportAnalyticsCompare:
    ...
    def test_report_not_found(self, mock_find: MagicMock, app: Flask) -> None:
        ...
        _register_routes(app)
        resp = app.test_client().get(
            "/api/report-analytics-compare?left=nonexistent&right=also_missing"
        )
        assert resp.status_code == 404
...
class TestApiReportResultDetail:
    ...
    def test_success(
        self, mock_find: MagicMock, mock_detail: MagicMock, app: Flask
    ) -> None:
        """成功返回结果详情。"""
        _register_routes(app)
